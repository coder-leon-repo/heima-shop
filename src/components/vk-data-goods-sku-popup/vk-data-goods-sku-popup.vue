...
export default {
  ...
  methods: {
    ...
    updateGoodsInfo(goodsInfo) {
      ...
      if (
        ...
        that.goodsInfo[that.goodsIdName] !==
          goodsInfo[that.goodsIdName]
      ) {
    ...
    async open() {
      ...
      if (
        that.customAction &&
        typeof that.customAction === 'function'
      ) {
        try {
  ...
  computed: {
    ...
    isManyCom() {
      let that = this
      let { goodsInfo, defaultSingleSkuName, specListName } =
        that
      let isMany = true
      if (
